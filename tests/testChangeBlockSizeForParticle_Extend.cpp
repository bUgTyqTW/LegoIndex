...
{
    double xStep = (blockEnv.xMax - blockEnv.xMin) / nSubBlocksEach;
    double yStep = (blockEnv.yMax - blockEnv.yMin) / nSubBlocksEach;
    double zStep = (blockEnv.zMax - blockEnv.zMin) / nSubBlocksEach;
...
int main(int argc, char *argv[])
{
    ...
    for (const auto &attributePair : attributesMap) {
        ...
        {   
            ...
            if (original_value == 930398351)
            {
                original_value = 930398002;
            }
    ...
    while (reader_engine.BeginStep() == adios2::StepStatus::OK)
    {
        ...
        for (x_it = xBlocksInfo.begin(); x_it != xBlocksInfo.end(); ++x_it, ++y_it, ++z_it, ++ux_it, ++uy_it, ++uz_it) {
            ...
            size_t total_blocks = var_vec1.size() / nSubBlocksEach + 1;
            ...
            for (size_t i = 0; i < var_vec1.size(); ++i) {
                ...
                {
                    blocksData_x[currentWriteBlock].push_back(data_x[j]);
                    blocksData_y[currentWriteBlock].push_back(data_y[j]);
                    blocksData_z[currentWriteBlock].push_back(data_z[j]);
                    blocksData_ux[currentWriteBlock].push_back(data_ux[j]);
                    blocksData_uy[currentWriteBlock].push_back(data_uy[j]);
                    blocksData_uz[currentWriteBlock].push_back(data_uz[j]);
                }

                if ((i + 1) % nSubBlocksEach == 0 || i == var_vec1.size() - 1)
                {
